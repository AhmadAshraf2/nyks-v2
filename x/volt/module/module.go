...
import (
	...
	"twilight-project/nyks/x/volt/keeper"
...
func (am AppModule) EndBlock(goCtx context.Context) error {
	...
	newSweepDetected, reserveId, roundId := am.keeper.CheckForNewSweepProposal(ctx)
	if !newSweepDetected {
		return nil
	}

	// Generate Withdrawal Snapshot
	am.generateWithdrawSnapshot(ctx, reserveId, roundId)

	// Generate Refund Transaction Snapshot
	am.generateRefundTxSnapshot(ctx, reserveId, roundId)

	return nil
}

func (am AppModule) generateWithdrawSnapshot(ctx sdk.Context, reserveId uint64, roundId uint64) {
	logger := am.keeper.Logger(ctx)

	pool, found := am.keeper.GetReserveWithdrawPool(ctx, reserveId)
	if !found {
	...
	for count, identifier := range pool.QueuedWithdrawIdentifiers {
		...
		withdrawRequest, found := am.keeper.GetBtcWithdrawRequestByIdentifier(ctx, identifier)
		if !found {
	...
	err := am.keeper.SetReserveWithdrawPool(ctx, pool)
	if err != nil {
	...
	store := am.keeper.Store(ctx)
	store.Set(snapshotKey, am.keeper.Codec().MustMarshal(&snapshot))
...
func (am AppModule) generateRefundTxSnapshot(ctx sdk.Context, reserveId uint64, roundId uint64) {
	logger := am.keeper.Logger(ctx)

	clearingAccounts, found := am.keeper.GetAllClearingAccountsInaReserve(ctx, reserveId)
	if !found {
